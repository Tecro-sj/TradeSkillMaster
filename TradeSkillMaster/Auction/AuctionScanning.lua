...
--    maxPages - maximum pages to scan (nil for unlimited)
function TSMAPI.AuctionScan:RunQuery(query, callbackHandler, resolveSellers, maxPrice, doCache, maxPages)
	...
	elseif not CanSendAuctionQuery() then
		TSMAPI:CreateTimeDelay("cantSendAuctionQueryDelay", 0.1, function() TSMAPI.AuctionScan:RunQuery(query, callbackHandler, resolveSellers, maxPrice, doCache, maxPages) end)
	...
	private.maxPages = maxPages or nil  -- Maximum pages to scan per query (nil = unlimited)
...
function private:ScanAuctions()
	...
	if private.scanType ~= "lastPage" then
		...
		if totalPages > 0 then
			-- Use the limited total if maxPages is set
			local effectiveTotal = totalPages
			if private.maxPages and totalPages > private.maxPages then
				effectiveTotal = private.maxPages
			end
			DoCallback("SCAN_PAGE_UPDATE", private.query.page, effectiveTotal)
		end
	end
	PopulatePageTemp()
	...
	elseif private.query.page >= totalPages then
		...
		private:StopScanning()
		return DoCallback("SCAN_COMPLETE", private.data)
	elseif private.maxPages and private.query.page >= private.maxPages then
		-- we have hit the max pages limit
		private:StopScanning()
		return DoCallback("SCAN_COMPLETE", private.data)
	end
