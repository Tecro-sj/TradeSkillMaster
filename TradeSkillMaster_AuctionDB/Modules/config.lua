	...
	local page = {
		{
			...
			children = {
				...
				{
					type = "InlineGroup",
					title = "Quick Scan Options",
					layout = "Flow",
					children = {
						{
							type = "EditBox",
							label = "Min Auctions per Item",
							value = TSM.db.profile.quickScanMinAuctions,
							relativeWidth = 0.45,
							callback = function(_, _, value)
								value = tonumber(value)
								if value and value >= 1 and value <= 100 then
									TSM.db.profile.quickScanMinAuctions = value
								else
									TSM:Print("Invalid value. Must be between 1 and 100.")
								end
							end,
							tooltip = "Quick Scan will only track items with at least this many auctions. Higher values = faster scans. Default: 3",
						},
						{
							type = "Label",
							relativeWidth = 0.1
						...
						{
							type = "EditBox",
							label = "Max Items to Scan",
							value = TSM.db.profile.quickScanMaxItems,
							relativeWidth = 0.45,
							callback = function(_, _, value)
								value = tonumber(value)
								if value and value >= 100 and value <= 10000 then
									TSM.db.profile.quickScanMaxItems = value
								else
									TSM:Print("Invalid value. Must be between 100 and 10000.")
								end
							end,
							tooltip = "Maximum number of items to include in Quick Scan. Quick Scan will scan the most traded items first. Lower values = faster scans. Default: 2000",
						},
						{
							type = "Label",
							text = "Quick Scan filters items from Full Scan results. Run a Full Scan first to update the Quick Scan list with these new settings.",
							relativeWidth = 1.0,
						},
					},
				},
