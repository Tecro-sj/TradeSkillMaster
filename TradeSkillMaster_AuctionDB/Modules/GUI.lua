...
function private:CreateStartScanContent(parent)
	...
	local function UpdateGetAllButton()
		if not frame.startGetAllButton then
			return  -- Do nothing if the Quick Scan feature is disabled.
		end

		if TSM.Scan.isScanning then
			frame:Disable()
		else
			-- Quick Scan is always ready (no cooldown like GetAll)
			frame:Enable()
	...
	if includeGetAll then
		...
		btn:SetText(L["Run Quick Scan"])
		btn.tooltip = L["A Quick Scan will scan individual items from your last Full Scan, but only the first 5 pages per item. This is much faster than a Full Scan and provides a good overview of market prices for all items. You must run a Full Scan first to build the item list."]
