...
Scan.quickScanStartTime = 0
Scan.quickScanSecondsPerItem = -1
Scan.quickScanCompleteElapsed = nil

local verifyNewAlgorithm = false  -- DEVELOPERS: Set to "true" to validate and benchmark the new market data algorithm!


local function QuickScanCallback(event, ...)
	-- This callback scans individual items with max 5 pages per item.
	-- After each item completes, it moves to the next item.
	if event == "SCAN_PAGE_UPDATE" then
		local page, total = ...

		-- Calculate the current page progress within this item
		local progress_float = page / total

		-- Calculate overall progress (item progress + within-item page progress)
		local item_base_progress = (Scan.quickScanCurrentIndex - 1) / Scan.quickScanTotalItems
		local item_page_progress = (1.0 / Scan.quickScanTotalItems) * progress_float
		local overall_progress = (item_base_progress + item_page_progress) * 100

		-- Calculate time estimate for Quick Scan
		local time_estimate_str = ""
		if Scan.quickScanCurrentIndex >= 1 and Scan.quickScanTotalItems > Scan.quickScanCurrentIndex then
			local seconds_elapsed = abs(time() - Scan.quickScanStartTime)
			local seconds_per_item = seconds_elapsed / Scan.quickScanCurrentIndex

			Scan.quickScanSecondsPerItem = seconds_per_item

			local progress_complete = Scan.quickScanCurrentIndex / Scan.quickScanTotalItems
			local remaining_complete = 1.0 - progress_complete

			local last_scan_seconds_per_item = TSM.db.realm.lastQuickScanSecondsPerItem
			if last_scan_seconds_per_item and last_scan_seconds_per_item > 0 then
				seconds_per_item = (seconds_per_item * progress_complete) + (last_scan_seconds_per_item * remaining_complete)
			end

			local seconds_total_estimate = max(1, floor((Scan.quickScanTotalItems * seconds_per_item) + 0.5))
		...
		-- Update status with item name and page info
		local itemName = Scan.quickScanQueue[Scan.quickScanCurrentIndex]
		local maxPages = min(total, Scan.maxPagesPerItem)
		local statusText = format("Quick Scan (%d/%d): %s [%d/%d]",
			Scan.quickScanCurrentIndex,
			Scan.quickScanTotalItems,
			itemName or "?",
			page, maxPages) .. time_estimate_str

		TSM.GUI:UpdateStatus(statusText, overall_progress)
	elseif event == "SCAN_COMPLETE" then
		-- This item is complete, merge the data and scan the next item
		local data = ...

		-- Merge this item's data into the overall quick scan data
		for itemString, obj in pairs(data) do
			if not Scan.quickScanData[itemString] then
				Scan.quickScanData[itemString] = obj
			else
				-- Merge records from this item into existing item data
				for _, record in ipairs(obj.records) do
					tinsert(Scan.quickScanData[itemString].records, record)
				end
			end
		end

		-- Move to next item
		Scan:QuickScanNextItem()
...
local function FullScanCallback(event, ...)
	...
	elseif event == "SCAN_COMPLETE" then
		...
		-- Extract and save item list for Quick Scan to use
		Scan:SaveItemListFromFullScan(data)
...
function Scan:SaveItemListFromFullScan(data)
	-- Extract unique item names from the full scan data and save for Quick Scan
	-- Only save items that have enough auctions to be worth scanning
	if not TSM.db.realm.quickScanItemList then
		TSM.db.realm.quickScanItemList = {}
	end
	wipe(TSM.db.realm.quickScanItemList)

	local minAuctions = TSM.db.profile.quickScanMinAuctions or 3
	local maxItems = TSM.db.profile.quickScanMaxItems or 2000

	-- First pass: collect all items with their auction counts
	local itemList = {}
	for itemString, obj in pairs(data) do
		...
		if itemID then
			...
			if itemName and itemName ~= "" then
				local auctionCount = #obj.records
				if auctionCount >= minAuctions then
					tinsert(itemList, {
						name = itemName,
						count = auctionCount
					})
	...
	-- Sort by auction count (most auctions first)
	...
	local itemCount = 0
	for i = 1, min(#itemList, maxItems) do
		TSM.db.realm.quickScanItemList[itemList[i].name] = true
		itemCount = itemCount + 1
	end

	TSM.db.realm.quickScanItemListTimestamp = time()
	TSM:Print(format("Saved %d items for Quick Scan (filtered from %d total, min %d auctions)", itemCount, #itemList, minAuctions))
end
...
function Scan:StartGetAllScan()
	-- Refuse to perform "Quick Scan" if we're called while "GetAll" is disabled.
	...
	-- Check if we have a saved item list from a previous Full Scan
	if not TSM.db.realm.quickScanItemList or not next(TSM.db.realm.quickScanItemList) then
		TSM:Print(L["Please run a Full Scan first to build the item list for Quick Scan."])
		return
	end

	-- Begin the "Quick Scan" - scans individual items with max 5 pages per item
	-- Uses the item list saved from the last Full Scan
	TSM.db.profile.lastGetAll = time()
	Scan.isScanning = "QuickScan"
	Scan.isBuggedGetAll = nil
	Scan.groupItems = nil
	Scan.maxPagesPerItem = 5  -- Limit to 5 pages per item
	TSMAPI.AuctionScan:StopScan()

	-- Build item scan queue from saved list
	Scan.quickScanQueue = {}
	Scan.quickScanData = {}
	Scan.quickScanCurrentIndex = 0
	Scan.quickScanTotalItems = 0

	for itemName, _ in pairs(TSM.db.realm.quickScanItemList) do
		tinsert(Scan.quickScanQueue, itemName)
		Scan.quickScanTotalItems = Scan.quickScanTotalItems + 1
	end

	-- Sort alphabetically for predictable scanning order
	table.sort(Scan.quickScanQueue)

	Scan.quickScanStartTime = time()
	Scan.quickScanSecondsPerItem = -1
	Scan.quickScanCompleteElapsed = nil

	local daysAgo = floor((time() - (TSM.db.realm.quickScanItemListTimestamp or 0)) / 86400)
	TSM:Print(format("Starting Quick Scan with %d items (list is %d days old)", Scan.quickScanTotalItems, daysAgo))

	-- Start scanning the first item
	Scan:QuickScanNextItem()
end

function Scan:QuickScanNextItem()
	Scan.quickScanCurrentIndex = Scan.quickScanCurrentIndex + 1

	if Scan.quickScanCurrentIndex > #Scan.quickScanQueue then
		-- All items scanned, process the collected data
		Scan:ProcessScanData(Scan.quickScanData)

		local elapsed = time() - Scan.quickScanStartTime
		Scan.quickScanCompleteElapsed = elapsed

		if Scan.quickScanSecondsPerItem > 0 then
			TSM.db.realm.lastQuickScanSecondsPerItem = Scan.quickScanSecondsPerItem
			TSM.db.realm.quickScanItemCount = Scan.quickScanTotalItems
		end

		Scan:DoneScanning(elapsed)

		-- Mark as complete scan
		TSM.db.realm.lastCompleteScan = time()
		return
	end
	...
	-- Update status with item name
	local statusText = format("Quick Scan (%d/%d): %s",
		Scan.quickScanCurrentIndex,
		Scan.quickScanTotalItems,
		itemName or "?")
	TSM.GUI:UpdateStatus(statusText, (Scan.quickScanCurrentIndex - 1) / Scan.quickScanTotalItems * 100)

	-- Scan this item with max 5 pages
	TSMAPI.AuctionScan:RunQuery(query, QuickScanCallback, nil, nil, nil, Scan.maxPagesPerItem)
end
